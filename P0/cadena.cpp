...
bool operator == (const Cadena& cadena1, const Cadena& cadena2) noexcept{
	return strcmp(cadena1, cadena2)==0;  // Se llama impicitamente a la conversión a char*
}
...
char Cadena::operator[] (size_t posicion) const{
	return s_[posicion];
}

char& Cadena::operator[] (size_t posicion){
	return s_[posicion];
}
